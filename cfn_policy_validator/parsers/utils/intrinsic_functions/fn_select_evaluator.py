...
class SelectEvaluator:
	...
	def evaluate(self, value, visited_nodes):
		...
		try:
			index = self.node_evaluator.eval(value[0], visited_nodes=visited_nodes)
			index = int(index)
		except ValueError:
			raise ApplicationError(f'The first value for Fn::Select must be an integer. Invalid value: {value[0]}')

		list_of_objects = self.node_evaluator.eval_with_validation(value[1], list_of_objects_schema, path='Fn::Select.1', visited_nodes=visited_nodes)
		...
		return self.node_evaluator.eval(list_of_objects[index], visited_nodes)
